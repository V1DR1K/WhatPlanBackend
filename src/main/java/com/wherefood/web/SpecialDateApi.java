...
record SpecialDateRequest(@NotNull LocalDate date, @NotBlank @Size(max = 160) String label) {}
record SpecialDateDto(Long id, LocalDate date, String label, Instant createdAt, Instant updatedAt) {}
...
public class SpecialDateApi {
 ...
 private static void apply(SpecialDate value, SpecialDateRequest request) { value.date = request.date(); value.label = request.label().trim(); }
 private static SpecialDateDto specialDate(SpecialDate value) { return new SpecialDateDto(value.id, value.date, value.label, value.createdAt, value.updatedAt); }
